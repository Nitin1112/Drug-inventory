...
const NewReceipt = () => {
    ...
    const fetchData = async () => {
      ...
      if (!response.error) {
        ...
        for (const med of response.data) {            
            if (med.stock > 0) {
                responseData.push(med);
            }
        }
        setAvailableMedicines(responseData);
  ...
  const handleItemNameChange = (itemNo, value) => {
    ...
    console.log("selectedMedicine: " + selectedMedicine.medicineOrDrug);
    setReceiptData((prevData) =>
      prevData.map((item) =>
        item.itemNo === itemNo
          ? {
              ...item,
              itemName: value,
              itemId: selectedMedicine.medicineOrDrug,
